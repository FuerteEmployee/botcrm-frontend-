...
import { useProjectService, type Project as BackendProject } from "@/services/project-service";
...
function ProjectPage() {
  ...
  const [form,     setForm]     = useState<Partial<BackendProject>>({ name: "", client: "", deadline: "", status: "" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", client: "", deadline: "", status: "" });
    setOpen(true);
  };
  const openEdit = (item: BackendProject) => {
    setEditing(item);
    setForm(item);
    setOpen(true);
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
          ...
          <DataTable columns={["Project Name", "Client", "Deadline", "Status", "Actions"]}>
            {filtered.map((item: any) => (
              <DataTableRow key={item._id}>
                ...
                <DataTableCell>{(item as any).client}</DataTableCell>
                <DataTableCell>{(item as any).deadline}</DataTableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? "Edit" : "Add"} Projects</DialogTitle>
